...
.run(function($rootScope,$state,$ionicHistory,$ionicPlatform,$ionicLoading,$window) {
      var tags = new Array();          
 ...
 /*$.ajax({
    url: 'http://bhavik.icoddertechnology.com/WireFrame/webservice/countrylist.php',
    type: 'POST',
...
.controller('changepasswordCtrl', function($scope,$http,$state) {
    ...
    $scope.changePassword = function(){
      ...
      if( password == null || newpassword == null){
        ...
        var req = {
            method: 'POST',
            url: 'http://bhavik.icoddertechnology.com/WireFrame/webservice/change_password.php',
            headers: {
...
.controller('forgotpassword', function($scope,$http,$state,$ionicPopup) {
    ...
    $scope.forgetPassword = function(){
      ...
      if(email == null){
        ...
        var req = {
            method: 'POST',
            url: 'http://bhavik.icoddertechnology.com/WireFrame/webservice/forgotpassword.php',
            headers: {
              'Content-Type': 'application/json'
            },
            data: {
...
.controller('signupCtrl', function($http,$scope,$state,$filter,$ionicLoading,$rootScope) {
  ...
  $scope.CreateUser = function(){
    ...
    $.ajax({
      type:"POST",
      url: 'http://bhavik.icoddertechnology.com/WireFrame/webservice/registration.php',
      data:{
        "action": "Registration",
        "firstname": firstName,
        "device_id": "XYZ123",
...
.controller('loginCtrl', function($rootScope,$scope,$http, $state, myservice, $ionicLoading, $ionicPopup) {
  ...
  $scope.login = function() {
    ...
    var device_id = "XYZ123";
    //var device_id = window.device.uuid;        
    ...
    if( email == null  || password == null){
      ...
      var req = {
        method: 'POST',
        url: 'http://bhavik.icoddertechnology.com/WireFrame/webservice/login.php',
        headers: {
          'Content-Type': 'application/json'
        },
        data: {
...
.controller('editProfileCtrl', function($http,$scope,$filter,$ionicLoading,$rootScope,$state) {
  ...
  $scope.loyalityCardTypeOpts = [
      {name:'Gold Card', value:'1'},
      {name:'Silver Card', value:'2'},
      {name:'Club Seven', value:'3'}      
    ];
  ...
  console.log(JSON.stringify(clientdetails));
    ...
    $scope.edit.dob = new Date(clientdetails.date_of_birth);
...
.controller('signCtrl', function($scope, $state, $http, $ionicPopup, Login, User, Camera, $filter, $localStorage){
  $scope.data = {};
  $scope.choseArea = {name:"India","areacode":"91"};
